...
// Generate Image using features array
export const POST = async (req) => {
  try {
    const { features } = await req.json();
    const newImage = {
      url: baseUrl,
      text: `a pixel art character with features: ${features.join(", ")}`,
      id: Math.floor(Math.random() * 1000),
      features: features,
    };

    return NextResponse.json({ ok: true, data: newImage });
  } catch (error) {
